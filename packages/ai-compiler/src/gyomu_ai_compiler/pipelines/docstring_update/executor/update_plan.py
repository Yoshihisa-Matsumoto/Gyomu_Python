...
async def generate_docstring_update_plan(
    ...
    prompt = load_prompt("docstring-update-base.md")
    if isinstance(prompt, Failure):
        return prompt

    conversation = ConversationSchema(
        system=MessageSchema.system_text(prompt.unwrap())
    ).with_request(MessageSchema.user_text(serialize_docstring_file_context(context)))
...
def serialize_docstring_file_context(
    context: DocstringFileContext,
) -> str:
    return json.dumps(
        (context),
